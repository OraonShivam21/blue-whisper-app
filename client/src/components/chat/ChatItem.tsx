...
const ChatItem: React.FC<{
  ...
  const deleteChat = async () => {
    await requestHandler(
      // A callback function that performs the deletion of a one-on-one chat its ID
      async () => await deleteOneOnOneChat(chat._id),
      null,
      // A callback function to be executed on success. It will call `onChatDelete` with the chat's ID as its parameter
      () => {
