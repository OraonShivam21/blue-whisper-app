...
const GroupChatDetailsModal: React.FC<{
  ...
  // State to manage list of users, initially set as an empty array
  ...
  const handleGroupNameUpdate = async () => {
    ...
    requestHandler(
      // Callback to update the group name with the provided chatId and newGroupName
      async () => await updateGroupName(chatId, newGroupName),
      null,
      // On successful update, set the new group details and other related stated
      (res) => {
        const { data } = res;
  ...
  const getUsers = async () => {
    requestHandler(
      // Callback to get the list of available users
  ...
  const deleteGroupChat = async () => {
    ...
    requestHandler(
      // Callback to delete the group using the provided chatId
  ...
  // Function to remove participant from a group chat
  const removeParticipant = async (participantId: string) => {
    requestHandler(
      // Main request function to remove a participant from the group
      async () => await removeParticipantFromGroup(chatId, participantId),
      null,
      // Callback after the request is successful
      () => {
        ...
        setGroupDetails(updateGroupDetails as ChatListItemInterface);

        // Inform the user chat that the participant has been removed
  ...
  const addParticipant = async () => {
    ...
    requestHandler(
      ...
      // Use the `alert` function as the callback error handler
      alert
    );
  };
  ...
  useEffect(() => {
    ...
    // Fetch group information and users when the modal or components opens
