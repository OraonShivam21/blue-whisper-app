...
const ChatMessage = require("../models/message.models");
const { emitSocketEvent } = require("../socket/index");
const ApiError = require("../utils/ApiError");
const ApiResponse = require("../utils/ApiResponse");
const asyncHandler = require("../utils/asyncHandler");
const { removeLocalFile } = require("../utils/helpers");
...
const createOrGetAOneOnOneChat = asyncHandler(async (req, res) => {
  ...
  if (!receiver) throw new ApiError(404, "Receiver does not exists");
  ...
  const chat = await Chat.aggregate([
    {
      $match: {
        isGroupChat: false,
        // avoid group chats, this controller is responsible for one on one chats
  ...
  if (chat.length) {
    return res.status(200).json(200, chat[0], "Chat retrieved successfully");
  }

  // if not we need to create a new one on one chat
  const newChatInstance = await Chat.create({
    name: "One on one chat",
    participants: [req.user._id, new mongoose.Types.ObjectId(receiverId)], // add receiver and logged in user as participant
    admin: req.user._id,
  });
  ...
  payload?.participants?.forEach((participant) => {
    ...
    emitSocketEvent(req, participant._id?.toString(), "newChat", payload);
  });
...
const createAGroupChat = asyncHandler(async (req, res) => {
  ...
  // check after removing the duplicate, we want group chat to have minimum 3 numbers including admin
  if (members.length < 3)
    throw new ApiError(
      400,
...
const getGroupChatDetails = asyncHandler(async (req, res) => {
  ...
  const gropuChat = await Chat.aggregate([
    {
      $match: {
...
const renameGroupChat = asyncHandler(async (req, res) => {
  ...
  payload?.participants?.forEach((participant) => {
    // emit event to all the participants with udpated chat as a payload
    emitSocketEvent(
      req,
...
const deleteGroupChat = asyncHandler(async (req, res) => {
  ...
  chat?.participants?.forEach((participant) => {
    ...
    emitSocketEvent(req, participant._id?.toString(), "leaveChat", chat);
  });
...
const deleteOneOnOneChat = asyncHandler(async (req, res) => {
  ...
  if (!payload) throw new ApiError(404, "Chat does not exists");
  ...
  emitSocketEvent(req, otherParticipant._id?.toString(), "leaveChat", payload);
